...
class LanguageServer:
    ...
    @classmethod
    def create(
        cls, config: MultilspyConfig, logger: MultilspyLogger, repository_root_path: str, add_gitignore_content_to_config: bool = True
    ) -> "LanguageServer":
        """
        ...
        if add_gitignore_content_to_config:
            ...
            if not os.path.exists(gitignore_path):
                logger.log(f"Should ignore all files in gitignore not not .gitignore found at {gitignore_path}. Skipping.", logging.WARNING)
    ...
    def __init__(
        ...
        self.ignore_spec = pathspec.PathSpec.from_lines(pathspec.patterns.GitWildMatchPattern, processed_patterns)
    ...
    def insert_text_at_position(
        ...
        file_buffer.contents = file_buffer.contents[:change_index] + text_to_be_inserted + file_buffer.contents[change_index:]
        ...
        if isinstance(response, list):
            ...
            for item in response:
                ...
                if LSPConstants.URI in item and LSPConstants.RANGE in item:
                    ...
                    try:
                        new_item["relativePath"] = str(PurePath(os.path.relpath(new_item["absolutePath"], self.repository_root_path)))
                    except:
    ...
    async def request_completions(
        ...
        with self.open_file(relative_file_path):
            ...
            while response is None or (response["isIncomplete"] and num_retries < 30):
                ...
                response: Union[List[LSPTypes.CompletionItem], LSPTypes.CompletionList, None] = await self.server.send.completion(
                    completion_params
                )
                if isinstance(response, list):
    ...
    async def request_document_symbols(
        ...
        def turn_item_into_symbol_with_children(item: GenericDocumentSymbol):
            ...
            if "location" not in item:
                absolute_path = (Path(self.repository_root_path) / relative_file_path).resolve()
    ...
    async def request_full_symbol_tree(
            ...
            for item in items:
                ...
                if os.path.isdir(abs_item_path):
                    try:
                        ...
                        self.logger.log(f"Error processing directory {rel_item_path}: {str(e)}", logging.WARNING)
                        continue

                elif os.path.isfile(abs_item_path):
                    try:
                        # Create a file symbol to hold document symbols
                        # Note: Use basename without extension for Python modules
                        file_basename = os.path.basename(item)
                        file_name = os.path.splitext(file_basename)[0]  # Remove extension

                        file_symbol = multilspy_types.UnifiedSymbolInformation(
                            name=file_name,  # Use name without extension
                            kind=multilspy_types.SymbolKind.File,
                            location=multilspy_types.Location(
                                uri=str(pathlib.Path(abs_item_path).as_uri()),
                                range={"start": {"line": 0, "character": 0}, "end": {"line": 0, "character": 0}},
                                absolutePath=str(abs_item_path),
                                relativePath=rel_item_path,
                            ),
                        ...
                        # Add the document symbols as children of the file symbol
                        file_symbol["children"].extend(root_nodes)

                        # Add the file symbol as a child of the package symbol
                        package_symbol["children"].append(file_symbol)
                    except MultilspyException as e:
                        # Skip files that can't be read (like binary files)
                        self.logger.log(f"Skipping file {rel_item_path}: {str(e)}", logging.WARNING)
                        continue
                    except Exception as e:
                        # Handle any other exceptions that might occur
                        self.logger.log(f"Error processing file {rel_item_path}: {str(e)}", logging.WARNING)
                        continue
        ...
        try:
            ...
            def process_symbol(symbol: multilspy_types.UnifiedSymbolInformation):
                try:
                    ...
                    # Skip any symbols that cause errors during processing
                    self.logger.log(f"Error processing symbol {symbol.get('name', 'unknown')}: {str(e)}", logging.WARNING)
                    return
            ...
            self.logger.log(f"Error generating directory overview for {relative_dir_path}: {str(e)}", logging.WARNING)
    ...
    async def request_referencing_symbols(
        ...
        for ref in references:
            ...
            with self.open_file(ref_path) as file_data:
                ...
                containing_symbol = await self.request_containing_symbol(ref_path, ref_line, ref_col, include_body=include_body)
    ...
    async def request_container_of_symbol(
    ...
    async def request_defining_symbol(
        ...
        defining_symbol = await self.request_containing_symbol(def_path, def_line, def_col, strict=False, include_body=include_body)
...
class SyncLanguageServer:
    ...
    def create(
        ...
        return SyncLanguageServer(
            LanguageServer.create(config, logger, repository_root_path, add_gitignore_content_to_config=add_gitignore_content_to_config)
        )
    ...
    def request_definition(self, file_path: str, line: int, column: int) -> List[multilspy_types.Location]:
        ...
        :return List[multilspy_types.Location]: A list of locations where the symbol is defined
        """
        result = asyncio.run_coroutine_threadsafe(self.language_server.request_definition(file_path, line, column), self.loop).result()
        return result
    ...
    def request_references(self, file_path: str, line: int, column: int) -> List[multilspy_types.Location]:
        ...
        """
        result = asyncio.run_coroutine_threadsafe(self.language_server.request_references(file_path, line, column), self.loop).result()
        return result
    ...
    def request_references_with_content(
        ...
        result = asyncio.run_coroutine_threadsafe(
            self.language_server.request_references_with_content(
                relative_file_path, line, column, context_lines_before, context_lines_after
            ),
            self.loop,
        ).result()
        return result
    ...
    def request_dir_overview(self, relative_dir_path: str) -> dict[str, list[tuple[str, multilspy_types.SymbolKind, int, int]]]:
        ...
        """
        assert self.loop
        result = asyncio.run_coroutine_threadsafe(self.language_server.request_dir_overview(relative_dir_path), self.loop).result()
        return result
    ...
    def request_document_overview(self, relative_file_path: str) -> list[tuple[str, multilspy_types.SymbolKind, int, int]]:
        ...
        """
        assert self.loop
        result = asyncio.run_coroutine_threadsafe(self.language_server.request_document_overview(relative_file_path), self.loop).result()
        return result
    ...
    def request_hover(self, relative_file_path: str, line: int, column: int) -> Union[multilspy_types.Hover, None]:
        ...
        :return None
        """
        result = asyncio.run_coroutine_threadsafe(self.language_server.request_hover(relative_file_path, line, column), self.loop).result()
        return result
    ...
    def request_parsed_files(self) -> list[str]:
        ...
        This seems to be the only way, the LSP does not provide any endpoints for listing project files."""
        assert self.loop
        result = asyncio.run_coroutine_threadsafe(self.language_server.request_parsed_files(), self.loop).result()
        return result
    ...
    def request_containing_symbol(
        ...
        assert self.loop
        result = asyncio.run_coroutine_threadsafe(
            self.language_server.request_containing_symbol(
                relative_file_path, line, column=column, strict=strict, include_body=include_body
            ),
            self.loop,
        ).result()
        return result
    ...
    def search_files_for_pattern(
        ...
        assert self.loop
        result = asyncio.run_coroutine_threadsafe(
            self.language_server.search_files_for_pattern(

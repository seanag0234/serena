...
from serena.agent import Tool, show_fatal_exception_safe
from serena.mode_tool import SetModesTool

# Import the extended SerenaAgent instead of the original
...
def create_mcp_server(
    ...
    :param context: Context name or path to context configuration file
    :param modes: List of mode names or paths to mode configuration files
    ...
    try:
        # Create the extended SerenaAgent with context and modes
        agent = SerenaAgent(
            project_file_path=project_file_path,
            context_name_or_path=context,
            mode_names_or_paths=modes
        )
        
        # Add the SetModesTool
        set_modes_tool = SetModesTool(agent)
...
@click.option(
    ...
    type=str,
    default=None,
    help="Context name or path to a context configuration file.",
)
@click.option(
    "--mode",
    "modes",
    ...
    multiple=True,
    default=None,
    help="Mode name or path to a mode configuration file. Can be specified multiple times for multiple modes.",
)
@click.option(
    "--transport",
...
def start_mcp_server(
    ...
    # Log the context and modes that are being used
    if context:
        log.info(f"Using context: {context}")
    if modes:
        log.info(f"Using modes: {', '.join(modes)}")
    
    # Create MCP server with context and modes
